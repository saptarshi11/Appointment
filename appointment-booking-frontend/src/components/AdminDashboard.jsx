...
const AdminDashboard = ({ user, onLogout }) => {
  ...
  const handleCancelBooking = async (bookingId, patientName) => {
    if (!window.confirm(`Are you sure you want to cancel the booking for ${patientName}?`)) {
      return
    }
                        ...
                        return (
                          <div key={booking.id} className="flex items-center justify-between p-4 border rounded-lg bg-white">
                            ...
                            <div className="flex items-center space-x-3">
                              ...
                              {isUpcoming && (
                                <button
                                  onClick={() => handleCancelBooking(booking.id, booking.user_name)}
                                  disabled={loading}
